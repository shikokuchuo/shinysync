...
vignette: >
  %\VignetteIndexEntry{Collaborative Kanban Board}
...
This vignette demonstrates how to build a collaborative kanban board using
shinysync's `kanban_ui()` and `kanban_server()` functions.
...
can't do this, leading to a frustrating experience.
...
When the UI re-renders after a remote change, you're not in the middle of
anything that gets disrupted.
...
The `doc_id` parameter identifies the collaborative document. All sessions
using the same `doc_id` synchronize together:
...
`kanban_server()` returns a reactive data frame with columns `id`, `text`,
`done`, and `column`:
...
For production deployments needing persistence and scaling, use `editor()`
with a sync server.
